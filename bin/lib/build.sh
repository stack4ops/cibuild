...
cibuild__build_get_sbom_args() {
  local build_sbom=$(cibuild_env_get 'build_sbom') \
        build_sbom_backend=$(cibuild_env_get 'build_sbom_backend') \
        build_client=$(cibuild_env_get 'build_client')

  # trivy backend — SBOM generated externally, no buildkit flags needed
  [ "${build_sbom_backend}" = "trivy" ] && return 0
...
cibuild__build_get_provenance_args() {
  ...
  # trivy backend — provenance not generated by buildkit
  # kept separate: provenance via sigstore/slsa-generator is future work
  [ "${build_sbom_backend}" = "trivy" ] && printf '%s\n' "" && return 0
...
cibuild__build_image_buildx() {
  ...
  for platform in ${platforms}; do
    ...
    # trivy SBOM — runs after push, scans from registry
    cibuild__build_sbom \
      "${platform_name}" \
...
EOF
  ...
  for platform in ${platforms}; do
    ...
    # --- SBOM ---
    # priority 1: nix-native SBOM output (packages.<s>.<attr>-sbom)
    #             used by ZenDIS-style distroless builds
    # priority 2: trivy on the pushed registry image (build_sbom_backend=trivy)
    nix_sbom_attr="${nix_flake_attr}-sbom"
    _nix_sbom_done=0

    if nix build ".#${nix_sbom_attr}" \
         --system "${nix_system}" \
         ${nix_opts} \
         --no-link 2>/dev/null; then
      cibuild_log_info "nix-native SBOM found for ${nix_sbom_attr}"
      nix_sbom_path=$(nix build ".#${nix_sbom_attr}" \
        --system "${nix_system}" \
        ${nix_opts} \
        --no-link \
        --print-out-paths 2>/dev/null)
      sbom_file=$(find "${nix_sbom_path}" -name "*.json" | head -n 1)
      if [ -n "${sbom_file}" ] && [ -n "${CIBUILD_OUTPUT_DIR:-}" ]; then
        cp "${sbom_file}" "${CIBUILD_OUTPUT_DIR}/sbom-${platform_name}.spdx.json"
        cibuild_log_info "nix-native SBOM written"
        _nix_sbom_done=1
      fi
    fi

    if [ "${_nix_sbom_done}" = "0" ]; then
      # trivy on registry image — consistent with other backends
      cibuild__build_sbom \
        "${platform_name}" \
        "${target_image}:${build_tag}-${platform_name}"
    fi
...
cibuild__build_sbom() {
  local platform_name=$1 \
        image_ref=$2 \
        build_sbom=$(cibuild_env_get 'build_sbom') \
        build_sbom_backend=$(cibuild_env_get 'build_sbom_backend') \
        build_sbom_format=$(cibuild_env_get 'build_sbom_format') \
        output_dir="${CIBUILD_OUTPUT_DIR:-}"

  [ "${build_sbom}" = "1" ]             || return 0
  [ "${build_sbom_backend}" = "trivy" ] || return 0
  ...
  # spdx-json → .spdx.json   cyclonedx-json → .cdx.json   spdx → .spdx
  case "${build_sbom_format}" in
  ...
  cibuild_log_info "generating SBOM via trivy (${build_sbom_format}) for ${platform_name}"

  trivy image \
    --format "${build_sbom_format}" \
    --output "${output_dir}/sbom-${platform_name}.${sbom_ext}" \
    --quiet \
    "${image_ref}" || \
    cibuild_log_err "trivy SBOM generation failed (non-fatal)"

  # vulnerability scan — write report, never fail the build
  trivy image \
    --format json \
    --output "${output_dir}/trivy-${platform_name}.json" \
    --quiet \
    "${image_ref}" || true

  cibuild_log_info "trivy SBOM + scan written to ${output_dir}"
}
